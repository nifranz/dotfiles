function d-exec
    ...
    # Check if --sh flag is passed and remove it from argv
    if contains -- --sh $argv
        set shell /bin/sh
        # Remove --sh from the arguments
        set argv (string join ' ' (string match -v -- '--sh' $argv))
    end
    ...
    # Run docker exec in the selected container with the appropriate shell
